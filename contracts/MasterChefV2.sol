...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    function safeChadTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > chadBal) {
            transferSuccess = chad.transfer(_to, chadBal);
        } else {
            transferSuccess = chad.transfer(_to, _amount);
        }
        require(transferSuccess, "safeChadTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _chadPerBlock) public onlyOwner {
        massUpdatePools();
        chadPerBlock = _chadPerBlock;
        emit UpdateEmissionRate(msg.sender, _chadPerBlock);
    }
}
